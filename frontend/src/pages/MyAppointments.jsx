...
const MyAppointments = () => {
    ...
    useEffect(() => {
        ...
        const capturePayment = async () => {
            if (appointmentId && paypalToken) {
                try {
                    const { data } = await axios.post(
                        backendUrl + '/api/user/payment-paypal/capture',
                        { token: paypalToken, appointmentId },
                        { headers: { token } }
                    );
                    if (data.success) {
                        toast.success(data.message);
                        getUserAppointments(); // Refetch appointments after successful payment
                        navigate('/my-appointments', { replace: true }); // Use replace to avoid back button issues
                        setSearchParams({});
                    } else {
                        toast.error(data.message);
        ...
        capturePayment();
    }, [searchParams, backendUrl, token, navigate, getUserAppointments, setSearchParams]);
